...
-- ПРИНЦИП РАБОТЫ --
    1. Находим искомый узел по ключу. Если узел не найден возвращаем Null.
    2. Если узел не имеет потомков, если узел - корень, то дерево очищается.
        Если нет - узел отсоединяется, от родителя.
    3. Если правого потомка нет узел заменяется левым поддеревом.
    4. Если левого потомка нет узел заменяется правым поддеревом.
    5. Если есть два потомка, узел заменяется преемником.

    Поиск приемника.
        Возвращает узел со следующим значением после передаваемого аргументом.
        Для этого он сначала переходим к правому потомку, а затем отслеживаем цепочку левых потомков этого узла.
        Для этого приемник ищется в самом нижнем левом листе правого дерева.

-- ДОКАЗАТЕЛЬСТВО КОРРЕКТНОСТИ --
Структура является бинарным деревом поиска, в котором хранятся ключи.
При удалении элемента из дерева, структура сохраняется.

-- ВРЕМЕННАЯ СЛОЖНОСТЬ --
Сложность удаления узла составляет O(h), где h –— высота дерева.

-- ПРОСТРАНСТВЕННАЯ СЛОЖНОСТЬ --
Сложность дерева составляет O(n).
